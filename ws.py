            ...
            if data["writing"]:
                if "change" in data:
                    try:
                        if compress:
                            compress_json.dump(
                                data, "./json/" + str(place).upper() + ".json.gz"
                            )
                        else:
        ...
        while True:
            ...
            if msg.startswith("list"):
                ...
                for file in sorted(glob.glob("./json/*")):
                    placelist.append(
                        os.path.basename(file).replace(".json", "").replace(".gz", "")
                    )
                ...
                users[client_id] = {"user": user, "pxs": int(userPx)}
                ...
                try:
                    if session in places:
                        ...
                        elif int(width) < int(places[session]["width"]) or int(
                            height
                        ) < int(places[session]["height"]):
                        ...
                        if poss in places[session]["data"]:
                            ...
                            if dt_diff <= delta_pxs:
                                await websocket.send(
                                    "place_error;The pixel is not yet available"
                                )
                                continue
                            ...
                            for ws in clients:
                                if ws != websocket:
                                    await ws.send(
                                        ...
                                        + poss
                                        + ";"
                                        + json.dumps(places[session]["data"][poss])
                                    )
                ...
                if session in places:
                    if user != places[session]["creator"]:
                        ...
                        try:
                            ...
                            for ws in clients:
                                await ws.send(
                                    "write_change;" + user + ";" + session + ";" + write
                                )
